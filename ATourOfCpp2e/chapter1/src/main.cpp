...
int count_x(const char*, char); // count the number of occurrences of x in p[];  p is assumed to point to a zero-ter minated array of char (or to nothing)
...
int main()
{
  ...
  double d3 = {2.7}; // initialize d3 to 2.3 using constructor (the = is optional with { ... })
  int d4 = d1; // simple assignment NOT PREFERRED, since 'narrowing conversion' would happen
  ...
  const int dmv0 = 17; // dmv0 is immutable and can be evaluated at either RUN OR COMPILE time
  constexpr int dmv = 17; // dmv is also immutable but must be evaluated at COMPILE time
  double sum(const vector<double>&); // sum will not be allowed to modify its argument
  ...
  // constexpr double max2 = 1.4*ce_square(var); // ERROR : the value of ‘var’ is not usable in a constant expression
  ...
  char x = *p; // *= contents of; *p is the object that p points to, that object is of type char and is being assigned to x
  ...
  for (auto i=0; i!=10; ++i) // auto infers type from the assigned value
    v2[i]=v1[i]; // note that v2[i]=v1[i] DOES NOT GIVE v2[i]==v1[i] because ??

  for (auto x:v1) // range-for-statement. Here, auto makes it easy to allow this code to be used for either double or int
    cout << x << '\n';

  for (auto& x : v2) // using reference operator &, we can add 1 to each x in v directly
    ++x;
